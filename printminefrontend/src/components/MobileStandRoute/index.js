...
class MobileStandRoute extends Component {
  ...
  getKeyChainData = async () => {
    ...
    let url = `http://localhost:3000/mobilestand`;
    if (productRating) {
    ...
    return (
      <CartContext.Consumer>
        {({ addToCart }) => (
          <>
            <NavbarRoute />
            <div className="mobile-stand-main-container">
              ...
              <ul className="mobile-stand-ul-list">
                {keyChainData.map((each) => (
                  <li className="each-mobile-stand-list" key={each.id}>
                    <div className="mobile-stand-container">
...
export default MobileStandRoute;
